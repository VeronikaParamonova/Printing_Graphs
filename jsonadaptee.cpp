...
DataSet JsonAdaptee::loadFromJson(const QString& filePath)
{
    ...
    //Определяем формат JSON и выгружаем
    if (doc.isArray())
    {
...
DataSet JsonAdaptee::parseArrayFormat(const QJsonArray& array, const QString& filePath)//если данные в виде [{"date": "...", "value": ...}]
{
    DataSet result;
    result.sourcePath = filePath;
    result.sourceType = getType();
...
DataSet JsonAdaptee::parseObjectFormat(const QJsonObject& object, const QString& filePath)//если данные в объекте
{
    DataSet result;
    result.sourcePath = filePath;
    result.sourceType = getType();
...
DataSet JsonAdaptee::parseKeyValueFormat(const QJsonArray& array) //разбираем массив, где каждый элемент это объект с парами ключ: значение. Мы не знаем заранее, как называются ключи, поэтому ищем их регистронезависимо.
{
    DataSet result;
