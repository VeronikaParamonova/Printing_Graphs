...
DataSet SqlAdaptee::loadFromDatabase(const QString& filePath)
{
    ...
    for (const auto& col : columns)
    {
        ...
        if (dateColumn.isEmpty() && (name.contains("date") || name.contains("time") || name.contains("datetime") || type.contains("DATE") || type.contains("DATETIME") || type.contains("TIMESTAMP")))
        {
            dateColumn = col.first;
        }

        // Ищем столбец для значения
        if (valueColumn.isEmpty() && (name.contains("value") || name.contains("temp") || name.contains("data") || name.contains("val") || type.contains("REAL") || type.contains("INTEGER") || type.contains("NUMERIC") || type.contains("FLOAT") || type.contains("DOUBLE")))
        {
            valueColumn = col.first;
        }
    }

    // Если не нашли столбцы, то используем первые два
    ...
    // Формируем запрос
    QString queryStr = QString("SELECT %1, %2 FROM %3").arg(dateColumn).arg(valueColumn).arg(tableName);
...
QList<PointData> SqlAdaptee::parseQueryResult(QSqlQuery& query) //преобразуем строки в QDateTime и double
{
