    ...
    , m_grayCheckBox(nullptr)
{
    ...
    //Проверяем, что контейнер получен
    if (!m_container) {
...
{
    QStringList filePaths = QFileDialog::getOpenFileNames(this, "Выберите файлы с данными", QDir::homePath(), "Файлы данных (*.db *.sqlite *.json);;Все файлы (*.*)");
    ...
    for (const QString& filePath : filePaths) {
        ...
        // Проверка формата
        if (extension != "db" && extension != "sqlite" && extension != "sqlite3" && extension != "json")
        {
            QString message = "Неподдерживаемый формат:" + filePath;
        ...
        if (!adapter)
        {
            QString message = "Нет адаптера для:" + filePath;
            showErrorMessage("Ошибка", message);
            continue;
        }
        ...
        if (data.isEmpty())
        {
            QString message = "Нет данных в:" + filePath;
            showErrorMessage("Ошибка", message);
            continue;
        }
    ...
    this->statusBar()->showMessage("Загружено файлов: " + QString::number(loadedCount) + " из " + QString::number(filePaths.size()));
...
void MainWindow::updateFileList()
{
    ...
    for (int i = 0; i < m_filePaths.size(); ++i)
    {
        ...
        double sizeKB = fileInfo.size() / 1024.0;
        QString fileSize = QString::number(sizeKB, 'f', 1) + " КБ";
        QString lastModified = fileInfo.lastModified().toString("dd.MM.yyyy HH:mm");

        displayStrings << QString("%1 | %2 | %3 | %4 точек").arg(fileInfo.fileName()).arg(fileSize).arg(lastModified).arg(m_fileData[i].size());
    }
...
void MainWindow::onChartTypeChanged(int index)
{
    ...
    // Если есть загруженные данные, то обновляем график
...
void MainWindow::updateChart(const DataSet& data, const QString& chartType)
{
    ...
    //получаем график из контейнера по имени
    auto chart = m_container->GetObjectByName<IChart>(chartType);
...
void MainWindow::updateStatusBar(const DataSet& data)
{
    ...
    QString statusText = QString("Выбран: %1 | Точек: %2 | Мин: %3 | Макс: %4 | Ср: %5").arg(data.name).arg(data.size()).arg(minVal, 0, 'f', 2).arg(maxVal, 0, 'f', 2).arg(avg, 0, 'f', 2);
...
void MainWindow::onPrintButtonClicked()
{
    ...
    QString filePath = QFileDialog::getSaveFileName(this, "Сохранить PDF", QDir::homePath() + "/график.pdf", "PDF файлы (*.pdf)"
    );
    ...
    //ЯВНО УСТАНАВЛИВАЕМ ОБЛАСТЬ РИСОВАНИЯ
