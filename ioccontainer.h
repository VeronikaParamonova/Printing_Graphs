...
public:
    ...
    std::shared_ptr<T> GetObject(const QString& param)
    {
        ...
        //Пробуем как AdapterFactory (условная фабрика с параметром)
        ...
        //Пробуем как CFactory (обычная фабрика без параметра)
    ...
    // Получение по имени
    template<typename T>
    std::shared_ptr<T> GetObjectByName(const QString& name)
    {
            int key = GetTypeID<T>() + qHash(name);//qHash вычисляет хэш-код для строки и потом этот код складывается с айди, чтоб получить уникальный ключ (чтобы не делать логику в лямбде как с графиками)
    ...
    // НОВОЕ регистрация с именем (для графиков)
    template<typename TInterface>
    void RegisterInstance(std::shared_ptr<TInterface> t, const QString& name)
    {
        int key = GetTypeID<TInterface>() + qHash(name); //qHash вычисляет хэш-код для строки и потом этот код складывается с айди, чтоб получить уникальный ключ (чтобы не делать логику в лямбде как с графиками)
        m_factories[key] = std::make_shared<CFactory<TInterface>>(
            [ = ] { return t; }
            );
    }
